...
class MCWindow(QMainWindow):
    ...
    def setFromOptions(self, options, args = None):
        ...
        # Color Selections
        for opt, wid in [(options['background'], self.ui.fileBackgroundB), (options['titlecolor'], self.ui.titleColorB)]:
    ...
    def run(self):
        ...
        for i in xrange(self.ui.dirsL.count()):
            ...
            try:
                make_contact.createContactSheet(self.options, d.text(), progress=self.progress)
            except Exception,e:
                break
